...
if(isset($_POST['rejistrar'])){
	...
	$consulta = "INSERT INTO usuarios (Nombre, ApellidoP, ApellidoM, Id_Genero, Email, Id_Tusuario, Usuario, password) 
	VALUES ('$Nombre','$Apellido1','$Apellido2','$genero','$gmail','$idT','$nick','$pas')";
	$guardarcon = $conecta->query($consulta);
	if($guardarcon < 0){
		$mensaje.= "<h3 class='text-danger'> Ocurrio un  error </h3>";
	}
	else{
		$mensaje.= "<h3 class='text-success'> Su registro fue exitoso</h3>";
	}
	} 
?>
<!DOCTYPE html>
...
<body>
		<img  src="img/VFast.jpg" alt="Logo Vfast" class="text-center" width="15%" height="15%" >
	<!--Es el comiemso del codigo -->
      <div class="container text-center">
			<p class="text-muted py-2"> <h4>V.Fast | Registrarse</h4></p>
			<p class="text-muted py-2">Agrega tus datos para el registro</p>
		</div>
         <!--Justificasion -->
				  ...
				  <div class="col-sm-10 col-md-12 col-lg-12">
                    ...
                    <form class="needs-validation"  action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
						...
						<div class="input-group mb-3">
							...
							<input type="password" name="Password" id="ver" class ="form-control " placeholder ="contraseña" aria-label ="pass" aria-describedaby="pass" required>
						...
						<div class="input-group mb-3">
							<span class="input-group-text" id="registro">
								<svg class="bi" width="32" height="38" fill="currentColor">
                                 <use xlink:href="main/icons/bootstrap-icons.svg#journal-text"/>
                                 </svg>
								
							</span>
							<input type="text" name="usuario" class ="form-control" placeholder ="nombre de usuario" aria-label ="text" aria-describedaby="text" required>
						...
						<div class="container text-center">
							<p class="text-muted text-dark py-0" >¿ya tienes cuenta?  inicia sesión <a href="index.php" class="link-dark"> AQUI </a></p>
						</div>
